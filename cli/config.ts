...
export class SeroConfig {
    ...
    constructor(credentials: Credentials, server: Server) {
    ...
    save(path: string): Result<void, SeroError> {
        const safe_save = Result.fromThrowable(
            fs.writeFileSync,
            SeroError.newfrom
        );
        ...
        return safe_save(
            path,
            TOML.stringify(JSON.parse(JSON.stringify(this)))
        );
    }
    ...
    static read(path: string): Result<SeroConfig, SeroError> {
        ...
        return read_sero_toml_contents_safe(path)
            .andThen((contents) =>
                parse_sero_toml_contents_safe(contents.toString())
            )
